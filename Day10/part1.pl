    ...
    for my $add_x (-1..1) {
        for my $add_y (-1..1) {
            ...
            if ($neighbor_height eq $height - 1) {
                unless (
                    exists $peaks_reachable{$neighbor_x} && 
                    exists $peaks_reachable{$neighbor_x}{$neighbor_y} &&
                    exists $peaks_reachable{$neighbor_x}{$neighbor_y}{$key}
                ) {
                    $peaks_reachable{$neighbor_x}{$neighbor_y}{$key} = 1;
...
foreach my $coords_ref (@trailheads) {
    ...
    my $reachable_ref = $peaks_reachable{$x}{$y};
    my %reachable = %$reachable_ref;
    my $score = keys %reachable;
    $total += $score;
}
